...
-- génère la colonne interet_paca
ALTER TABLE statuts.method_enjeu_paca
DROP COLUMN IF EXISTS interet_paca;

ALTER TABLE statuts.method_enjeu_paca
ADD COLUMN interet_paca TEXT GENERATED ALWAYS AS (
  CASE
    WHEN dd_rate >= 5 THEN 'DD'
    WHEN 
      COALESCE(point_indigenat, 0) +
      COALESCE(point_lrn, 0) +
      COALESCE(point_znieff, 0) +
      ...
      COALESCE(point_eee, 0) +
      COALESCE(point_enjeu_cbn, 0) +
      COALESCE(point_biodiv, 0) <= -1 THEN 'NUL'
    WHEN 
      COALESCE(point_indigenat, 0) +
      COALESCE(point_lrn, 0) +
      COALESCE(point_znieff, 0) +
      ...
      COALESCE(point_eee, 0) +
      COALESCE(point_enjeu_cbn, 0) +
      COALESCE(point_biodiv, 0) <= 1 THEN 'TRES FAIBLE'
    WHEN 
      COALESCE(point_indigenat, 0) +
      COALESCE(point_lrn, 0) +
      COALESCE(point_znieff, 0) +
      ...
      COALESCE(point_eee, 0) +
      COALESCE(point_enjeu_cbn, 0) +
      COALESCE(point_biodiv, 0) <= 4 THEN 'FAIBLE'
    WHEN 
      COALESCE(point_indigenat, 0) +
      COALESCE(point_lrn, 0) +
      COALESCE(point_znieff, 0) +
      ...
      COALESCE(point_eee, 0) +
      COALESCE(point_enjeu_cbn, 0) +
      COALESCE(point_biodiv, 0) <= 9 THEN 'MODERE'
    WHEN 
      COALESCE(point_indigenat, 0) +
      COALESCE(point_lrn, 0) +
      COALESCE(point_znieff, 0) +
      ...
      COALESCE(point_eee, 0) +
      COALESCE(point_enjeu_cbn, 0) +
      COALESCE(point_biodiv, 0) <= 15 THEN 'FORT'
    WHEN 
      COALESCE(point_indigenat, 0) +
      COALESCE(point_lrn, 0) +
      COALESCE(point_znieff, 0) +
      ...
      COALESCE(point_eee, 0) +
      COALESCE(point_enjeu_cbn, 0) +
      COALESCE(point_biodiv, 0) <= 29 THEN 'TRES FORT'
    ELSE 'MAJEUR'
  END
) STORED;

--Le faire avec une fonction
CREATE OR REPLACE FUNCTION statuts.mettre_a_jour_scores_trigger()
RETURNS trigger AS $$
BEGIN
  NEW.total_point :=
    COALESCE(NEW.point_indigenat, 0) +
    COALESCE(NEW.point_lrn, 0) +
    COALESCE(NEW.point_znieff, 0) +
    COALESCE(NEW.point_lrr, 0) +
    COALESCE(NEW.point_eee, 0) +
    COALESCE(NEW.point_enjeu_cbn, 0) +
    COALESCE(NEW.point_biodiv, 0);

  NEW.interet_paca := CASE
    WHEN NEW.dd_rate >= 4 THEN 'DD'
    WHEN NEW.total_point <= -1 THEN 'NUL'
    WHEN NEW.total_point <= 1 THEN 'TRES FAIBLE'
    WHEN NEW.total_point <= 4 THEN 'FAIBLE'
    WHEN NEW.total_point <= 9 THEN 'MODERE'
    WHEN NEW.total_point <= 15 THEN 'FORT'
    WHEN NEW.total_point <= 29 THEN 'TRES FORT'
    ELSE 'MAJEUR'
  END;
...
-- Lancer automatiquement la fonction à la mise à jour
DROP TRIGGER IF EXISTS trigger_maj_scores ON statuts.method_enjeu_paca;

CREATE TRIGGER trigger_maj_scores
BEFORE INSERT OR UPDATE OF 
  point_indigenat, point_lrn, point_znieff, point_lrr, 
  point_eee, point_enjeu_cbn, point_biodiv, dd_rate
ON statuts.method_enjeu_paca
FOR EACH ROW
EXECUTE FUNCTION statuts.mettre_a_jour_scores_trigger();
