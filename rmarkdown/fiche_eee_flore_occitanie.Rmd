...
ft <- flextable(df) |>
  ...
  add_header_row(values = c("Nom vernaculaire", "Nom scientifique", "Statuts"),
                 colwidths = c(1, 1, 3)) |>
  valign(part = "header", valign = "middle") |>
  theme_booktabs() |>
  autofit()
...
ft <- width(ft, j = 5, width = 1.8)   # 1.8*2 = 3.6
...
ft <- valign(ft, i = i_2rows, j = c(1, 4), valign = "middle", part = "body")
...
witdth_img = 4.5
height_img = 4.5 * info$height/info$width
...
if (file.exists(map_file)) {
  ft <- compose(ft, i = i_photo, j = 4,
                value = as_paragraph(as_image(map_file, width = 5,height = 3)),
                part = "body")
} else {
  ft <- compose(ft, i = i_photo, j = 4,
...
carte_legend_text <- "Répartition du taxon – Données SILENE"
...
# 
# bord_img <- fp_border(color = "#C9D6F1", width = 1)
# 
# ft <- border(
#   ft, i = i_photo, j = 1, part = "body",
#   border = bord_img  # toutes les faces (top/left/bottom/right)
# )
# ft <- border(
#   ft, i = i_photo, j = 4, part = "body",
#   border = bord_img
# )
...
ft <- valign(ft, j = names(df), valign = "middle", part = "header")
